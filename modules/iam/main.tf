# ---------------------------------------------------------------------------
# IAM Module — EKS cluster role, node group role, OIDC provider, and IRSA
# roles for ALB Controller, Cluster Autoscaler, and EBS CSI Driver.
# ---------------------------------------------------------------------------
...
locals {
  ...
  # Strip the https:// prefix from the OIDC URL for condition strings
...
# Inline policy — sourced from AWS official docs for the LBC
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = ["arn:${local.partition}:elasticloadbalancing:*:*:targetgroup/*/*", "arn:${local.partition}:elasticloadbalancing:*:*:loadbalancer/net/*/*", "arn:${local.partition}:elasticloadbalancing:*:*:loadbalancer/app/*/*"]
      },
      {
        Effect = "Allow"
        Action = [
